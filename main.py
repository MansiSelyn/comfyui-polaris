...
if args.zluda and not (args.use_quad_cross_attention or args.use_split_cross_attention or args.use_pytorch_cross_attention):
    # ZLUDA's shims produce NaN through torch's SDPA path; quad cross attention
    # avoids it and keeps the VAE decode clean too.
    args.use_quad_cross_attention = True
...
def handle_comfyui_manager_unavailable():
    msg = ("\n\nTo use the `--enable-manager` feature, the `comfyui-manager` package must be installed first.\n"
           "Run install.bat (it installs the MANAGER section of requirements.txt into .venv).\n")
    logging.warning(msg)
