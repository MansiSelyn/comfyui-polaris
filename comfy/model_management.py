...
def _enumerate_gpu_names():
    """Best-effort list of attached display adapter names (Windows only)."""
    if os.name != "nt":
        return set()
    try:
        import ctypes
        ...
        while True:
            ...
            if not user32.EnumDisplayDevicesA(None, device_index, ctypes.byref(device_info), 0):
                break
...
def _amd_gpu_detected():
    for name in _enumerate_gpu_names():
        lower = name.lower()
        if "radeon" in lower or "amdx" in lower:
            return True
    return False
...
if args.zluda:
    # Explicit ZLUDA launch (python main.py --zluda). ZLUDA exposes the HIP/ROCm
    # runtime through CUDA DLL shims, so torch is a normal CUDA build and the
    # standard CUDA code paths apply unchanged.
    # ZLUDA has no cublasLt (the shims only cover cublas/cusparse/nvrtc), so
    # torch's cublasLt addmm-with-bias path fails on gfx803 with
    # CUBLAS_STATUS_NOT_SUPPORTED. Route addmm through legacy cublas instead.
    os.environ.setdefault("DISABLE_ADDMM_CUDA_LT", "1")
    # ZLUDA's shims also fail scalar (0-dim) device<->host transfers: .item() and
    # the float()/int()/bool() conversions on a CUDA tensor raise a bogus
    # "CUDA error: out of memory". Route them through .cpu(), whose full-tensor
    # copy path works, before reading the value back.
    _zluda_item = torch.Tensor.item
    _zluda_tolist = torch.Tensor.tolist
    torch.Tensor.item = lambda self: _zluda_item(self.cpu())
    torch.Tensor.tolist = lambda self: _zluda_tolist(self.cpu())
    torch.Tensor.__float__ = lambda self: float(_zluda_item(self.cpu()))
    torch.Tensor.__int__ = lambda self: int(_zluda_item(self.cpu()))
    torch.Tensor.__index__ = lambda self: int(_zluda_item(self.cpu()))
    torch.Tensor.__bool__ = lambda self: bool(_zluda_item(self.cpu()))
    # cuDNN is the NVIDIA wheel's binary; against ZLUDA the first conv raises
    # CUDNN_STATUS_INTERNAL_ERROR, so route convolutions through the native path.
    torch.backends.cudnn.enabled = False
    if torch.cuda.is_available():
        logging.info("Using ZLUDA with device: {}".format(torch.cuda.get_device_name(0)))
    else:
...
if _POLARIS_ACTIVE:
    ...
    try:
        ...
        logging.error("--directml was requested, but torch_directml is not installed. Run install.bat (DIRECTML section, .venv); torch-directml and a CUDA torch cannot share one environment.")
        raise SystemExit(1)
...
elif os.getenv("COMFYUI_POLARIS_AUTO_DIRECTML", "1") == "1" and not torch.cuda.is_available() and torch.version.hip is None:
    # AMD Polaris (gfx803, RX 4xx/5xx) is not supported by ROCm. On Windows the
    # only GPU backend available for it is DirectML, so enable it automatically.
    if _amd_gpu_detected():
        try:
            import torch_directml
        except ImportError:
            pass
            ...
            directml_enabled = True
            directml_device = torch_directml.device()
            logging.info("Detected AMD GPU, using DirectML with device: {}".format(torch_directml.device_name(directml_device.index)))
            lowvram_available = False


def _directml_total_memory():
    # torch_directml does not expose the adapter's VRAM, so get_total_memory()
    # used a 1 GB placeholder and the memory manager underbudgeted on Polaris
    # cards. Query the real value from the D3D12 adapter (DXGI) instead.
    if _directml_total_memory.cache is not None:
        return _directml_total_memory.cache
    try:
        import ctypes
        ...
        hr = create_factory(ctypes.byref(iid), ctypes.byref(factory))
        if hr < 0 or not factory.value:
            return 0

        factory_vtbl = ctypes.cast(factory, ctypes.POINTER(ctypes.POINTER(ctypes.c_void_p))).contents
        release = ctypes.CFUNCTYPE(ctypes.c_ulong, ctypes.c_void_p)(factory_vtbl[2])

        enum_adapters1 = ctypes.CFUNCTYPE(ctypes.c_long, ctypes.c_void_p, wintypes.UINT, ctypes.POINTER(ctypes.c_void_p))
        adapter = ctypes.c_void_p()
        hr = ctypes.cast(factory_vtbl[12], enum_adapters1)(factory, 0, ctypes.byref(adapter))
        if hr < 0 or not adapter.value:
            release(factory)
            return 0

        adapter_vtbl = ctypes.cast(adapter, ctypes.POINTER(ctypes.POINTER(ctypes.c_void_p))).contents
        release_adapter = ctypes.CFUNCTYPE(ctypes.c_ulong, ctypes.c_void_p)(adapter_vtbl[2])
        desc = _DXGI_ADAPTER_DESC1()
        get_desc1 = ctypes.CFUNCTYPE(ctypes.c_long, ctypes.c_void_p, ctypes.POINTER(_DXGI_ADAPTER_DESC1))
        hr = ctypes.cast(adapter_vtbl[10], get_desc1)(adapter, ctypes.byref(desc))
        release_adapter(adapter)
        release(factory)
        if hr < 0:
            return 0
        _directml_total_memory.cache = int(desc.DedicatedVideoMemory)
    except Exception:
        _directml_total_memory.cache = 0
    return _directml_total_memory.cache


_directml_total_memory.cache = None
...
def get_total_memory(dev=None, torch_total_too=False):
    ...
    if hasattr(dev, 'type') and (dev.type == 'cpu' or dev.type == 'mps'):
        ...
        if directml_enabled:
            mem_total = _directml_total_memory() or (1024 * 1024 * 1024)
            mem_total_torch = mem_total
        elif is_intel_xpu():
...
def get_torch_device_name(device):
    if hasattr(device, 'type'):
        ...
        elif directml_enabled and device.type == "privateuseone":
            try:
                name = torch_directml.device_name(-1 if device.index is None else device.index)
            except:
                name = "DirectML"
            return "{} {} : {}".format(device, name, "directml")
        else:
...
def get_free_memory(dev=None, torch_free_too=False):
    ...
    if hasattr(dev, 'type') and (dev.type == 'cpu' or dev.type == 'mps'):
        ...
        if directml_enabled:
            mem_free_total = _directml_total_memory() or (1024 * 1024 * 1024)
            mem_free_torch = mem_free_total
        elif is_intel_xpu():
            stats = torch.xpu.memory_stats(dev)
